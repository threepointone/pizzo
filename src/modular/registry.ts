...
export const MODULES: Record<ModuleType, ModuleDef> = {
  ...
  sampleHold: {
    ...
    category: "control",
    color: "#84cc16",
    blurb: "Stepped random or sampled CV. Great for analog burble and unstable motion.",
    inputs: [
      { id: "in", label: "cv in", kind: "cv" },
      { id: "trig", label: "trig", kind: "cv" },
    ],
    outputs: [{ id: "out", label: "cv", kind: "cv" }],
    params: [
      {
  ...
  sequencer: {
    ...
    category: "control",
    color: "#f59e0b",
    blurb: "Eight-step CV pattern with a gate output for arps, riffs, and rhythmic modulation.",
    inputs: [],
    outputs: [
      { id: "cv", label: "cv", kind: "cv" },
      { id: "gate", label: "gate", kind: "cv" },
    ],
    params: [
      {
      ...
      { id: "steps", label: "Steps", kind: "number", min: 1, max: 8, step: 1, default: 8 },
      {
        id: "gate",
        label: "Gate",
        kind: "number",
      ...
      { id: "s1", label: "1", kind: "number", min: -1, max: 1, step: 0.01, default: 0 },
      { id: "s2", label: "2", kind: "number", min: -1, max: 1, step: 0.01, default: 0.25 },
      { id: "s3", label: "3", kind: "number", min: -1, max: 1, step: 0.01, default: 0.5 },
      { id: "s4", label: "4", kind: "number", min: -1, max: 1, step: 0.01, default: 0.25 },
      { id: "s5", label: "5", kind: "number", min: -1, max: 1, step: 0.01, default: -0.25 },
      { id: "s6", label: "6", kind: "number", min: -1, max: 1, step: 0.01, default: 0 },
      { id: "s7", label: "7", kind: "number", min: -1, max: 1, step: 0.01, default: 0.75 },
      { id: "s8", label: "8", kind: "number", min: -1, max: 1, step: 0.01, default: 0 },
    ],
  },
  slew: {
    type: "slew",
    ...
    category: "control",
    color: "#a3e635",
    blurb: "Glide for CV. Smooth pitch, random, and sequencer movement.",
    inputs: [{ id: "in", label: "cv in", kind: "cv" }],
    outputs: [{ id: "out", label: "cv", kind: "cv" }],
    params: [
      {
        id: "time",
  ...
  wavefolder: {
    ...
    category: "audio",
    color: "#fb7185",
    blurb: "West-coast-ish folding shaper for metallic basses, bells, and leads.",
    inputs: [{ id: "in", label: "audio", kind: "audio" }],
    outputs: [{ id: "out", label: "audio", kind: "audio" }],
    params: [
      { id: "fold", label: "Fold", kind: "number", min: 0.5, max: 12, step: 0.1, default: 3 },
      { id: "bias", label: "Bias", kind: "number", min: -1, max: 1, step: 0.01, default: 0 },
      { id: "level", label: "Level", kind: "number", min: 0, max: 1, step: 0.01, default: 0.65 },
    ],
  },
  chorus: {
    type: "chorus",
    ...
    category: "audio",
    color: "#60a5fa",
    blurb: "Modulated micro-delay for wide pads, keys, and synthetic ensembles.",
    inputs: [{ id: "in", label: "audio", kind: "audio" }],
    outputs: [{ id: "out", label: "audio", kind: "audio" }],
    params: [
      {
      ...
      { id: "depth", label: "Depth", kind: "number", min: 0, max: 1, step: 0.01, default: 0.45 },
      {
        id: "delay",
        label: "Delay",
        kind: "number",
      ...
      { id: "mix", label: "Mix", kind: "number", min: 0, max: 1, step: 0.01, default: 0.35 },
    ],
  },
  reverb: {
    type: "reverb",
